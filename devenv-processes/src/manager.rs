...
/// The phase to surface for a `Stopped` entry, given the terminal phase it
/// reached on its own (if any) and whether the user explicitly stopped it.
///
/// For display (`for_dependency = false`: lists, run summaries, the TUI) an
/// explicit user stop reads as a plain `Stopped`, even if the process had
/// exited — the user's stop is its final word. For dependency evaluation
/// (`for_dependency = true`) the terminal phase is always surfaced, so a
/// dependent on `<proc>@started` still sees that a process which ran did in
/// fact start, and is not stranded by an explicit stop.
fn stopped_phase(
    terminal_phase: Option<ProcessPhase>,
    user_stopped: bool,
    for_dependency: bool,
) -> ProcessPhase {
    if user_stopped && !for_dependency {
...
/// ([`stopped_phase`]): the terminal phase is recorded either way, so a process
    ...
    fn process_info(name: &str, entry: &ProcessEntry) -> ProcessInfo {
        let (phase, restart_count) = match entry {
            ...
            ProcessEntry::Stopped {
                ...
                user_stopped,
                ..
            } => (stopped_phase(*terminal_phase, *user_stopped, false), 0),
